...
public class OrdenService implements IOrdenService {
    ...
    public List<OrdenDTO> findByEstado(EstadoOrden estado) {
        return ordenRepository.findByEstado(estado).stream().map(OrdenMapper::toDTO).toList();
    }
    ...
    public OrdenDTO registerOrUpdate(OrdenDTO ordenDTO) {
        ...
        if (clienteOpt.isEmpty() || mesaOpt.isEmpty() || userOpt.isEmpty() || ordenDTO.getDetalle() == null ||
        ordenDTO.getDetalle().isEmpty()){
            return  null; //gestionar el controlador
        }
        ...
        if (ordenDTO.getId() == null){
            ...
            //logica oara actualizar datos de una orden
            ...
            orden.setMesa(mesaOpt.get());
            orden.setUsuario(userOpt.get());
            // limojamos el detalle previo
            orden.getDetalleOrden().clear();
        }
        //agregando al detalle de orden
        for (DetalleOrdenDTO detalleDTO: ordenDTO.getDetalle()){
            ...
            //agregamos que el detalleOrden ala orden
    ...
    private String generarCorrelativo() {
        ...
        String yearMonth =
                curentDate.format(DateTimeFormatter.ofPattern("yyyyMM"));
        Long maxCorrelativo =
                ordenRepository.findMaxCorrelativoByYearMonth(yearMonth);
